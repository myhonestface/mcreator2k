...
public class DiscordClient implements Closeable {
	...
	public DiscordClient() {
		...
		try {
			...
			handlers.ready = (user) -> LOG.debug("Conectado con " + user.username);
			handlers.disconnected = (errorCode, message) -> LOG.debug("Desconectado: " + errorCode + " - " + message);
			handlers.errored = (errorCode, message) -> LOG.warn(errorCode + " - " + message);
			discordRpc.Discord_Initialize("1018283287338295427", handlers, true, null);
